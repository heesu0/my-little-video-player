...
AudioBuffer::AudioBuffer(uint64_t size, uint64_t pts) : buffer_(new uint8_t[size]), size_(size), pts_(pts)
{
}
...
AudioBuffer::AudioBuffer(const AudioBuffer& audio_buffer)
    : buffer_(new uint8_t[audio_buffer.size_]), size_(audio_buffer.size_), pts_(audio_buffer.pts_)
{
...
AudioBuffer::AudioBuffer(AudioBuffer&& audio_buffer) noexcept
    : buffer_(audio_buffer.buffer_), size_(audio_buffer.size_), pts_(audio_buffer.pts_)
{
...
uint8_t* AudioBuffer::buffer() const
{
    return buffer_;
}

uint64_t AudioBuffer::size() const
{
    return size_;
}

uint64_t AudioBuffer::pts() const
{
    return pts_;
}
